...
export function HomeDashboard({
  ...
  const topCard =
    [...cardInvoices].sort(
      (left, right) => right.invoiceTotal - left.invoiceTotal,
    )[0] ?? null;
    ...
    >
      <div className="space-y-8">
        <section className="grid gap-8 xl:grid-cols-[320px_1fr]">
          <section>
            <h2 className="text-[1rem] font-bold uppercase text-slate-900 dark:text-slate-100">
              Totais:
            </h2>
        ...
        <section className="grid gap-8 xl:grid-cols-3">
          <div
            className="rounded-md border border-slate-300 bg-white p-3 dark:border-slate-700 dark:bg-slate-900"
            title="A pagar"
          >
            <SummaryRow
              label="Maior grupo"
              value={payableBuckets[0]?.label ?? "Sem pendências"}
            />
            <SummaryRow
              label="Valor"
              value={
                payableBuckets[0]
                  ? formatCurrency(payableBuckets[0].total)
                  : "-"
              }
            />
            <SummaryRow
              ...
              value={
                topCard
                  ? `${topCard.creditCard.name} • ${formatCurrency(topCard.invoiceTotal)}`
                  : "Sem fatura aberta"
              }
              icon
            />
          ...
          >
            <SummaryRow
              label="Maior grupo"
              value={receivableBuckets[0]?.label ?? "Sem valores pendentes"}
            />
            <SummaryRow
              label="Valor"
              value={
                receivableBuckets[0]
                  ? formatCurrency(receivableBuckets[0].total)
                  : "-"
              }
            />
          </div>
          ...
          >
            <SummaryRow
              label="Maior grupo"
              value={expenseBuckets[0]?.label ?? "Sem contas pagas"}
            />
            <SummaryRow
              label="Valor"
              value={
                expenseBuckets[0]
                  ? formatCurrency(expenseBuckets[0].total)
                  : "-"
              }
            />
          </div>
